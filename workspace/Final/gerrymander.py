...
class State(MultiPolygon):
	...
	def __init__(self,path, county=None):
         if (county!=None):
             from shapefile import Reader
             r = Reader(path)
             self._precincts = tuple(Precinct(shapeRec) for shapeRec in r.iterShapeRecords() if shapeRec.record[1] in county)
         else:
             from shapefile import Reader
             r = Reader(path)
...
class Precinct(Polygon):
	...
	def adjacent(self):
		...
		self._adjacent = tuple(p for p in self.state.iterPrecincts() if ((p.position() != self.position()) and len(points.intersection(p.points())) > 1))
		return self._adjacent
